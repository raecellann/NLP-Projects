...
class TypingTest:
    ...
    def load_corpus(self):
        ...
        for alt in [
            ...
            'corpora/eng_sentences.txt',
        ]:
            candidates.append(os.path.join(base_dir, alt))
        ...
        for path in candidates:
            if os.path.exists(path):
                ...
                if ext.lower() == '.csv':
                    self.ngram_model.load_corpus_from_csv(path)
                else:
                    self.ngram_model.load_corpus_from_text(path)
                loaded = True
                break
        ...
        if not loaded:
            ...
            if ext.lower() == '.csv':
                ...
                self.ngram_model.load_corpus_from_text(self.corpus_path)
