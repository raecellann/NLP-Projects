...
class TypingGame:
    ...
    def setup_ui(self):
        ...
        # Results buttons
        self.play_again_button = ModernButton(center_x, 650, button_width, button_height, "Play Again", SUCCESS_GREEN, SUCCESS_DARK, "🎮", 36)
        self.back_to_menu_button = ModernButton(center_x, 750, button_width, button_height, "Back to Menu", PRIMARY_BLUE, SECONDARY_BLUE, "🏠", 36)
    ...
    def draw_menu(self):
        ...
        # Modern title with enhanced glow effect
        title_text = "N-Gram Typing Challenge"
        self.title_glow += 0.08
        
        # Multiple glow layers for depth
        for i in range(6):
            glow_intensity = int(30 + 20 * math.sin(self.title_glow + i * 0.3))
            glow_color = (glow_intensity, glow_intensity, 255 - glow_intensity)
            glow_surface = self.font_title.render(title_text, True, glow_color)
            glow_rect = glow_surface.get_rect(center=(SCREEN_WIDTH // 2 + i*2, 180 + i*2))
            self.screen.blit(glow_surface, glow_rect)
        
        # Main title
        title_surface = self.font_title.render(title_text, True, PURE_WHITE)
        title_rect = title_surface.get_rect(center=(SCREEN_WIDTH // 2, 180))
        self.screen.blit(title_surface, title_rect)
        
        # Modern subtitle with animation
        subtitle_text = "Master your typing skills"
        subtitle_glow = int(120 + 40 * math.sin(self.title_glow * 1.5))
        subtitle_surface = self.font_small.render(subtitle_text, True, (subtitle_glow, subtitle_glow, subtitle_glow))
    ...
    def draw_game(self):
        ...
        if self.is_typing and self.total_chars > 0:
            ...
            info_panels.append((f"📊 {accuracy:.1f}%", SUCCESS_GREEN, SCREEN_WIDTH - 350, 145))
        
        for text, color, x, y in info_panels:
            panel_rect = pygame.Rect(x, y, 330, 50)
            panel_surf = pygame.Surface((330, 50), pygame.SRCALPHA)
        ...
        typing_area = pygame.Rect(60, 180, SCREEN_WIDTH - 120, 500)
