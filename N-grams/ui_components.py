"""
UI Components for the N-Grams Typing Test application.
Contains button classes and other UI elements.
"""
...
class ModernButton:
    ...
    def __init__(self, x: int, y: int, width: int, height: int, text: str, 
                 color: Tuple[int, int, int] = PRIMARY_BLUE, 
                 hover_color: Tuple[int, int, int] = SECONDARY_BLUE,
...
class OutlineButton:
    """Outline styled button with hover effects."""
...
class ButtonFactory:
    """Factory class for creating different types of buttons."""
    
    @staticmethod
    def create_menu_buttons():
        """Create all menu buttons with proper positioning."""
        row_button_width, row_button_height = BUTTON_DIMENSIONS["row_button"]
        gap = UI_SPACING["gap"]
        start_x = MENU_WIDTH // 2 - (3 * row_button_width + 2 * gap) // 2
        row_y = 360
        
        easy_button = OutlineButton(start_x, row_y, row_button_width, row_button_height, 
                                   "Easy Mode", SUCCESS_GREEN, 26)
        medium_button = OutlineButton(start_x + row_button_width + gap, row_y, 
                                     row_button_width, row_button_height, 
                                     "Medium Mode", PRIMARY_BLUE, 26)
        hard_button = OutlineButton(start_x + (row_button_width + gap) * 2, row_y, 
                                   row_button_width, row_button_height, 
                                   "Hard Mode", ERROR_RED, 26)

        second_gap = UI_SPACING["second_gap"]
        start_x2 = MENU_WIDTH // 2 - (2 * row_button_width + second_gap) // 2
        row2_y = row_y + 90
        
        custom_button = OutlineButton(start_x2, row2_y, row_button_width, row_button_height, 
                                     "Time: 60s", ACCENT_BLUE, 26)
        exit_button = OutlineButton(start_x2 + row_button_width + second_gap, row2_y, 
                                   row_button_width, row_button_height, 
                                   "Exit", NEUTRAL_GRAY, 26)
    ...
    def create_game_buttons():
        ...
        button_width, button_height = BUTTON_DIMENSIONS["game_button"]
        
        restart_button = ModernButton(60, 60, button_width, button_height, 
                                     "Restart", WARNING_ORANGE, WARNING_DARK)
        menu_button = ModernButton(210, 60, button_width, button_height, 
                                  "Menu", NEUTRAL_GRAY, NEUTRAL_DARK)
    ...
    def create_results_buttons():
        """Create results screen buttons."""
        button_width, button_height = BUTTON_DIMENSIONS["results_button"]
        spacing = UI_SPACING["results_spacing"]
        total_width = button_width * 2 + spacing
        start_x = GAME_WIDTH // 2 - total_width // 2
        y = GAME_HEIGHT - 150

        play_again_button = ModernButton(start_x, y, button_width, button_height,
                                        "Play Again", SUCCESS_GREEN, SUCCESS_DARK)
        back_to_menu_button = ModernButton(start_x + button_width + spacing, y,
                                          button_width, button_height,
                                          "Back to Menu", PRIMARY_BLUE, SECONDARY_BLUE)
        
        return {
            "play_again": play_again_button,
            "back_to_menu": back_to_menu_button
        } 